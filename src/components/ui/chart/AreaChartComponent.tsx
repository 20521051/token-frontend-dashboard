...
interface ChartComponentProps {
  setTradingData: React.Dispatch<React.SetStateAction<{ price: number; quantity: number; time: Date }[]>>;
}

const ChartComponent: React.FC<ChartComponentProps> = ({ setTradingData }) => {
  const [data, setData] = useState<{ name: Date; marketPrice: number; tokenVNSePrice: number; tradingValue: number }[]>([]);

  useEffect(() => {
    const interval = setInterval(() => {
      const currentTime = new Date();
      const newMarketPrice = faker.random.number({ min: 1.000, max: 10.000 });
      const newTokenVNSePrice = faker.random.number({ min: 100.0, max: 80.0 });
      const newTradingValue = faker.random.number({ min: 167.00, max: 175.00 }) / 100.0;
  ...
  return (
    <div className="absolute w-full h-422.188 bg-gray-900 rounded-lg shadow-md left-0 top-0" style={{ width: '800.781px' }}>
      ...
      <div className="w-full h-40">
        <ResponsiveContainer width="140%" height="180%">
          <ComposedChart data={data}>
            ...
            <XAxis dataKey="name" tickFormatter={(value: Date) => value.toLocaleTimeString('en-US', { hour: 'numeric', minute: 'numeric' })} />
